...
export const RevenueRouterPage: React.FC = () => {
  ...
  const [fractionalRevenue, setFractionalRevenue] = useState<any[]>([]);
  const [activeTab, setActiveTab] = useState<'overview' | 'history' | 'sources' | 'fractional'>('overview');
  ...
  const loadRouterData = async () => {
    try {
      setLoading(true);
      const [statsRes, historyRes, sourcesRes, fractionalRes] = await Promise.all([
      ...
      if (fractionalRes.ok) {
        const fractionalData = await fractionalRes.json();
        setFractionalRevenue(fractionalData.properties || []);
      }
    } catch (error) {
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="bg-white rounded-t-xl shadow-lg">
            ...
            <div className="p-6">
              ...
              {/* Fractional Properties Tab */}
              {activeTab === 'fractional' && (
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-4">Fractional Real Estate Revenue</h3>
                  <p className="text-gray-600 mb-6">
                    Monthly rental income distribution from fractional properties with tier-based revenue bonuses
                  </p>

                  {fractionalRevenue.length === 0 ? (
                    <div className="text-center py-12 text-gray-500">
                      <div className="text-6xl mb-4">🏢</div>
                      <p>No fractional properties yet. Properties will appear once deals are fractionalized.</p>
                    </div>
                  ) : (
                    <div className="space-y-6">
                      {/* Summary Stats */}
                      <div className="grid md:grid-cols-4 gap-4 mb-6">
                        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-lg p-4 border border-purple-200">
                          <div className="text-sm text-purple-700 mb-1">Total Properties</div>
                          <div className="text-3xl font-bold text-purple-900">{fractionalRevenue.length}</div>
                        </div>
                        <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-lg p-4 border border-blue-200">
                          <div className="text-sm text-blue-700 mb-1">Total Value</div>
                          <div className="text-3xl font-bold text-blue-900">
                            ${(fractionalRevenue.reduce((sum, p) => sum + p.property_value, 0)).toLocaleString()}
                          </div>
                        </div>
                        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 border border-green-200">
                          <div className="text-sm text-green-700 mb-1">Monthly Income</div>
                          <div className="text-3xl font-bold text-green-900">
                            ${(fractionalRevenue.reduce((sum, p) => sum + p.net_monthly_income, 0)).toLocaleString()}
                          </div>
                        </div>
                        <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-lg p-4 border border-orange-200">
                          <div className="text-sm text-orange-700 mb-1">Shares Sold</div>
                          <div className="text-3xl font-bold text-orange-900">
                            {((fractionalRevenue.reduce((sum, p) => sum + p.shares_sold, 0) / fractionalRevenue.reduce((sum, p) => sum + p.total_shares, 0)) * 100).toFixed(1)}%
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        {fractionalRevenue.map((property) => {
                          const fundingPercent = (property.shares_sold / property.total_shares) * 100;
                          const annualYield = ((property.net_monthly_income * 12) / property.property_value) * 100;
                            ...
                            <div key={property.id} className="bg-white border border-gray-200 rounded-lg p-6">
                              ...
                              <div className="grid md:grid-cols-5 gap-4">
                                ...
                                <div className="bg-blue-50 p-3 rounded">
                                  <div className="text-xs text-blue-700 mb-1">Monthly Income</div>
                                  <div className="text-lg font-bold text-blue-900">
                                    ${property.net_monthly_income.toLocaleString()}
                                  </div>
                                </div>
                                <div className="bg-green-50 p-3 rounded">
                                  <div className="text-xs text-green-700 mb-1">Annual Yield</div>
                                  <div className="text-lg font-bold text-green-900">
                                    {annualYield.toFixed(1)}%
                                  </div>
                                </div>
                                <div className="bg-purple-50 p-3 rounded">
                                  <div className="text-xs text-purple-700 mb-1">Funding</div>
                                  <div className="text-lg font-bold text-purple-900">
                                    {fundingPercent.toFixed(1)}%
                                  </div>
                                </div>
                                <div className="bg-orange-50 p-3 rounded">
                                  <div className="text-xs text-orange-700 mb-1">Reserve Fund</div>
                                  <div className="text-lg font-bold text-orange-900">
                                    {property.reserve_fund_percent}%
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="mt-4 pt-4 border-t border-gray-200">
                                ...
                                <div className="flex items-center gap-2 text-xs">
                                  ...
                                  <div className="bg-orange-100 text-orange-800 px-3 py-1 rounded-full">
                                    {property.reserve_fund_percent}% Reserve Fund
                                  </div>
                                </div>
                              </div>
                            </div>
                      ...
                      {/* Tier Explanation */}
                      <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg p-6 border border-blue-200 mt-6">
                        <h4 className="text-lg font-semibold text-gray-900 mb-4">4-Tier Investor Model</h4>
                        <div className="grid md:grid-cols-4 gap-4">
                          <div className="bg-white p-4 rounded-lg border border-blue-200">
                            <div className="font-semibold text-blue-800 mb-1">Retail</div>
                            <div className="text-xs text-gray-600 mb-2">$500 - $10,000</div>
                            <div className="text-2xl font-bold text-blue-600">+0%</div>
                          </div>
                          <div className="bg-white p-4 rounded-lg border border-purple-200">
                            <div className="font-semibold text-purple-800 mb-1">Accredited</div>
                            <div className="text-xs text-gray-600 mb-2">$10K - $100K</div>
                            <div className="text-2xl font-bold text-purple-600">+2%</div>
                          </div>
                          <div className="bg-white p-4 rounded-lg border border-orange-200">
                            <div className="font-semibold text-orange-800 mb-1">Premium</div>
                            <div className="text-xs text-gray-600 mb-2">$100K - $500K</div>
                            <div className="text-2xl font-bold text-orange-600">+5%</div>
                          </div>
                          <div className="bg-white p-4 rounded-lg border border-red-200">
                            <div className="font-semibold text-red-800 mb-1">Institutional</div>
