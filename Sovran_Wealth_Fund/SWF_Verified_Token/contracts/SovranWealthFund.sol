...
contract SovranWealthFund is ERC20Burnable, AccessControl, Pausable, IPegManagement {
    ...
    constructor() ERC20("Sovran Wealth Fund", "SWF") {
        ...
        _setupRole(PEG_MANAGER_ROLE, msg.sender);
    ...
    function getBasketValue() public view override returns (uint256 basketValue) {
        ...
        for (uint i = 0; i < assetSymbols.length; i++) {
            ...
            if (asset.active) {
                ...
                uint256 assetValue = assetPrice.mul(asset.weight).div(10000);
                basketValue = basketValue.add(assetValue);
            }
        }
    ...
    function stake(uint256 amount) external whenNotPaused {
        ...
        uint256 amountPerWallet = amount.div(info.virtualWallets.length);
        for (uint i = 0; i < info.virtualWallets.length; i++) {
            address virtualWallet = info.virtualWallets[i];
            info.walletBalances[virtualWallet] = info.walletBalances[virtualWallet].add(amountPerWallet);
        }
    ...
    function withdraw(uint256 amount) external {
        ...
        for (uint i = 0; i < info.virtualWallets.length; i++) {
            address virtualWallet = info.virtualWallets[i];
            info.walletBalances[virtualWallet] = info.walletBalances[virtualWallet].sub(amountPerWallet);
        }
    ...
    function getPendingRewards(address user) public view returns (uint256) {
        ...
        uint256 timeElapsed = block.timestamp.sub(info.lastUpdateTime);
