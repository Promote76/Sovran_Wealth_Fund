...
# Deployment script for Polygon Mainnet that can resume after interruptions
echo "Starting SWF deployment to Polygon Mainnet with resume capability..."
...
if [ -f "mainnet-deployment-progress.json" ]; then
  echo "Found deployment progress file. Resuming from previous state..."
else
  echo "Starting new deployment process..."
fi

# Run the deployment script
npx hardhat run deploy-mainnet-progress.js --network polygon

# Check if deployment was successful
if [ $? -eq 0 ]; then
  # Check if the progress file indicates completion
  if grep -q '"stage":"complete"' mainnet-deployment-progress.json; then
    echo "Deployment completed successfully!"
    echo "You can now verify the contracts on PolygonScan by running:"
    echo "./verify-contracts.sh"
  else
    echo "Deployment incomplete. You can resume it by running this script again."
  fi
else
  echo "Deployment script encountered an error. Please check the error messages above."
  exit 1
fi