...
async function main() {
  try {
    ...
    const increasedGasPrice = currentGasPrice.mul(200).div(100); // Double the current gas price
    
    console.log(`Current gas price: ${ethers.utils.formatUnits(currentGasPrice, "gwei")} gwei`);
    console.log(`Increased gas price (200%): ${ethers.utils.formatUnits(increasedGasPrice, "gwei")} gwei`);
    ...
    console.log("\nSending mint transaction with high gas price...");
    const mintTx = await tokenContract.mint(recipientAddress, amountInWei, {
      gasPrice: increasedGasPrice,
      gasLimit: 200000 // Explicitly set gas limit to ensure it goes through
    });
    ...
    if (error.message.includes("nonce too low")) {
      console.log("\nThe transaction nonce is too low. This usually means:");
      console.log("1. A transaction with this nonce has already been mined");
      console.log("2. You need to wait for pending transactions to complete");
      
      console.log("\nYou can check your pending transactions with:");
      console.log(`npx hardhat run scripts/checkTransaction.js --network polygon <tx-hash>`);
    }
  }
}
