// Script for deploying the minimal SovranWealthFundMinimal token to Polygon Mainnet
...
async function main() {
  try {
    ...
    console.log("POLYGON MAINNET DEPLOYMENT - MINIMAL CONTRACT");
    console.log("-".repeat(50));
    console.log("WARNING: You are deploying to the MAINNET, which will cost real POL tokens.");
    console.log("This script uses the minimal version of the contract to save on gas fees.");
    ...
    if (parseFloat(balanceInPOL) < 0.5) {
      console.error("\nWARNING: Your balance is below 0.5 POL. This may not be enough for deployment.");
    ...
    console.log("\nPreparing to deploy the SovranWealthFundMinimal token...");
    ...
    // Estimate gas cost (assume ~3.5M gas for deployment of minimal contract)
    const estimatedGasLimit = 3500000;
    ...
    // Deploy contract
    console.log("\nDeploying SovranWealthFundMinimal token contract...");
    const SovranWealthFundMinimal = await ethers.getContractFactory("SovranWealthFundMinimal");
    const token = await SovranWealthFundMinimal.deploy({
    ...
    const fs = require("fs");
    fs.writeFileSync(
      "mainnet-minimal-deployment.json", 
      JSON.stringify({
    ...
    console.log("\nDeployment information saved to mainnet-minimal-deployment.json");
